...
function Company() {
    ...
    const viewDatavalues = () => {
        axios.get("http://localhost:9001/api/get/endrolment")
    ...
    const saveDataChange = async () => {
        if (saveUpdateVal === "Save") {
            if (validform()) {
                ...
                try {
                    const res = await axios.post("http://localhost:9001/upload/images", formdate)
                    setResultText(res.data.message);
                    console.log(res, "123response");
                    payload["photo"] = res.data.message;
                    const postiamge = await axios.post("http://localhost:9001/api/insert/endrolment", payload)
            ...
            try {
                const postiamge = await axios.post("http://localhost:9001/api/updatevalue/data", payload)
                console.log(postiamge, "imageval");
    ...
    const deleteOnclickVal = (id) => {
        if (window.confirm("Are you sure want to delete")) {
            axios.delete(`http://localhost:9001/api/delete/value/${id}`, {
        ...
        <div className="fullview">
            <div className="heightpic" id='myDivtwo'>
                <div id='myDivtwo'>
                    ...
                    {/* <div class="navigate">
                        <ul >
                            <li><a className="arefer" href="/">Home</a></li>
